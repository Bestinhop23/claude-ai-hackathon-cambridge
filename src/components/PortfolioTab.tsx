...
import { Briefcase, Loader2, Plus, Trash2, Search } from "lucide-react";
...
const PortfolioTab = () => {
  ...
  const pieData = enriched.map((h, i) => ({ name: h.symbol, value: h.marketValue, color: COLORS[i % COLORS.length] }));
    ...
    <div className="space-y-4">
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        <div className="flex items-center gap-2 px-4 py-3 border-b border-border bg-secondary/30">
          ...
          <Button size="sm" variant="outline" className="ml-auto h-7 text-xs" onClick={() => setShowAdd(!showAdd)}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-border">
                {enriched.map(h => (
                  <tr key={h.id} className="hover:bg-secondary/30 transition-colors cursor-pointer" onClick={() => navigate(`/stock/${h.symbol}`)}>
                    ...
                    <td className="px-3 py-2.5">
                      ...
                      }`}>{ASSET_TYPE_LABELS[h.asset_type]}</span>
                    </td>
                    ...
                    <td className={`text-right px-3 py-2.5 font-mono font-semibold ${h.pnl >= 0 ? "text-stock-up" : "text-stock-down"}`}>
                      {h.pnl >= 0 ? "+" : ""}{currSym}{h.pnl.toFixed(0)} ({h.pnl >= 0 ? "+" : ""}{h.pnlPct.toFixed(1)}%)
                    </td>
...
function AddHoldingForm({ onAdded }: { onAdded: () => void }) {
  ...
  const [assetType, setAssetType] = useState<"equity" | "bond" | "commodity">("equity");
    ...
    <div className="px-4 py-3 border-b border-border bg-secondary/10 space-y-3">
      <div className="flex gap-2 items-center flex-wrap">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <Input
            placeholder="Search symbol (e.g. AAPL, GC=F, TLT)"
        ...
        <select value={assetType} onChange={e => setAssetType(e.target.value as any)} className="h-8 text-xs rounded-md border border-input bg-background px-2">
          <option value="equity">Equity</option>
          <option value="bond">Bond</option>
          <option value="commodity">Commodity</option>
        </select>
        <Input placeholder="Shares" type="number" className="w-20 h-8 text-xs" value={shares} onChange={e => setShares(e.target.value)} />
        <Input placeholder="Avg Cost" type="number" className="w-24 h-8 text-xs" value={avgCost} onChange={e => setAvgCost(e.target.value)} />
        <Button size="sm" className="h-8 text-xs" onClick={handleSave} disabled={saving || !selectedSymbol}>
